...
It is preffered to keep the data - rather than just remove the zeros - as the means or SDs of controls and treatments that contain zeros, make up XX of the data.


```{r}
#first lets see how many zeros there are:
meta_zeros <- meta %>% 
        filter(MeanControl==0.0 | MeanTreatment==0.0 | SDControl==0.0 | SDTreatment ==0.0)
# XX rows with zeros in means or SDs

#Adding 0.5 to everything!
meta_05 <- meta %>% 
        mutate_at(vars(c("SDControl", "SDTreatment", "MeanControl", "MeanTreatment")),
                  ~.+0.5)
