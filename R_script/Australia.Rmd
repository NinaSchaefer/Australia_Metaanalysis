...
min(meta_dens[,7][which(meta_dens[,7]>0)]) #control
min(meta_dens[,11][which(meta_dens[,11]>0)]) #treatment
#-> 0.125 smallest value -> use 0.120 to replace zeros
min(meta_dens[,10][which(meta_dens[,10]>0)]) #control
min(meta_dens[,14][which(meta_dens[,14]>0)]) #treatment
...
meta_dens$MeanTreatment[meta_dens$MeanTreatment == 0] <- 0.120 
meta_dens$MeanControl[meta_dens$MeanControl == 0] <- 0.120 
meta_dens$SDTreatment[meta_dens$SDTreatment == 0] <- 0.30 
meta_dens$SDControl[meta_dens$SDControl == 0] <- 0.30 
...
#add value slightly smaller than lowest mean and sd where there's a zero
meta_abund$MeanTreatment[meta_abund$MeanTreatment == 0] <- 0.075 
meta_abund$MeanControl[meta_abund$MeanControl == 0] <- 0.075 
meta_abund$SDTreatment[meta_abund$SDTreatment == 0] <- 0.180 
meta_abund$SDControl[meta_abund$SDControl == 0] <- 0.180 

#putting meta back together
meta <- rbind(meta_dens, meta_abund)
```
...
Parameter_orch <-
  orchard_plot(rom.FeatureStatus, mod = "Parameter", xlab = "Ratio of means", group="Study_Experiment", data=meta) + theme(axis.text.y = element_text(angle = 0, hjust = 1))
Parameter_orch
...
Parameter_cat <-
  caterpillars(rom.FeatureStatus, mod = "Parameter", xlab = "Ratio of means", group="Study_Experiment", data=meta)
FeatureStatus_cat
...
#                                                            test = "t",
#                                                            data = meta,
#                                                            control = list(optimizer ="bobyqa")) #added as error occured with optim with “Nelder-Mead”, “BFGS”,  did not work. have not tried “CG”, “L-BFGS-B”, “SANN”, “Brent”)
# 
# sum.het <- summary(rom.FeatureStatus.het)
# sum.het
# 
