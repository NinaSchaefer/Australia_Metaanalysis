...
```{r}
meta<-read.csv("data/Dataset_Australia_new_adjusted.csv",header=T, fileEncoding="latin1") #read in data

#adjusted dataset has orientations removed for vertical up and vertical down compariasons
...
meta$TaxonomicGroup_Status <-
  paste(meta$TaxonomicGroup, meta$Status.AUS, sep = "_")
meta$TaxonomicGroup_Status <-as.factor(meta$TaxonomicGroup_Status)
...
meta$Feature_Species_Status <-
  paste(meta$Feature_Species, meta$Status.AUS, sep = "_")
meta$Feature_Species_Status <-as.factor(meta$Feature_Species_Status)
...
levels(meta$Feature_Species_Status)
```
...
#checking how many studies are in each level:
TaxonomicGroup_Status_levels<-levels(meta$TaxonomicGroup_Status)
TaxonomicGroup_Status_EffectID<- map(TaxonomicGroup_Status_levels, ~length(unique(meta$Effect_ID[meta$TaxonomicGroup_Status ==.])))
TaxonomicGroup_Status_Study_Experiment<- map(TaxonomicGroup_Status_levels, ~length(unique(meta$Study_Experiment[meta$TaxonomicGroup_Status ==.])))
TaxonomicGroup_Status_Study_Location<- map(TaxonomicGroup_Status_levels, ~length(unique(meta$Study_Location[meta$TaxonomicGroup_Status ==.])))

meta_reduced <- subset(meta, TaxonomicGroup_Status!="Anthozoans_N") 
meta_reduced <- subset(meta_reduced, TaxonomicGroup_Status!="Bivalves_N") 
meta_reduced <- subset(meta_reduced, TaxonomicGroup_Status!="Bivalves_NI") 
meta_reduced <- subset(meta_reduced, TaxonomicGroup_Status!="Poriferans_N") 
...
## Adding Mod TAXONOMIC GROUP:STATUS
(could not be tested if only location and deployment used as "study" due to insufficient replication)

```{r, results='hide', fig.show='hide'}
#using reduced dataset where each level comprises of at least 3 experiments
rom.TaxonomicGroupStatus <- rma.mv(yi, V_reduced, mods = ~ TaxonomicGroup_Status - 1,
                                              random = list(~ 1|Study,
                                                            ~ 1|Effect_ID,
                                                            ~ 1|Species,
                                                            ~ 1|Phylogeny),
...
sum <- summary(rom.TaxonomicGroupStatus)
sum_I2 <- i2_ml(rom.TaxonomicGroupStatus)
sum_R2 <- r2_ml(rom.TaxonomicGroupStatus)

res.TaxonomicGroupStatus <-data.frame(effectname = attr(sum$beta, "dimnames")[[1]],
                          beta = as.numeric(sum$beta),
                          percentage_increase_or_decrease_on_treatment = (exp(sum$beta)-1)*100,
                          real.effect.value = exp(sum$beta),
                          pval = sum$pval,
                          ...
                          Moderator_significance_stat = sum$QM,
                          Moderator_significance_pval = sum$QMp,
                          AIC = rom.TaxonomicGroupStatus$fit.stats$REML[3])


res_TaxonomicGroupStatus_df <- bind_rows(res.TaxonomicGroupStatus)
write.csv(res_TaxonomicGroupStatus_df, file = "./output/summ_TaxonomicGroupStatus.csv")


TaxonomicGroupStatus_orch <-
  orchard_plot(rom.TaxonomicGroupStatus, mod = "TaxonomicGroup_Status", xlab = "Ratio of means", group="Study_Experiment", data=meta_reduced) + theme(axis.text.y = element_text(angle = 0, hjust = 1))
TaxonomicGroupStatus_orch

ggsave(
 TaxonomicGroupStatus_orch,
  file = "./output/TaxonomicGroupStatus_orch.tiff",
  dpi = plot_dpi,
  width = 10,
  height = 8
)

TaxonomicGroupStatus_cat <-
  caterpillars(rom.TaxonomicGroupStatus, mod = "TaxonomicGroup_Status", xlab = "Ratio of means", group="Study_Experiment", data=meta_reduced)
TaxonomicGroupStatus_cat
ggsave(TaxonomicGroupStatus_cat, file = "./output/TaxonomicGroupStatus_cat.tiff", dpi = plot_dpi)


table(meta_reduced$TaxonomicGroup_Status)
contrast_TaxonomicGroupStatus <- summary(glht(
  rom.TaxonomicGroupStatus,
  linfct = c(
    "TaxonomicGroup_StatusPolychaetes_N - TaxonomicGroup_StatusPolychaetes_NI = 0",
    "TaxonomicGroup_StatusBryozoans_N - TaxonomicGroup_StatusBryozoans_NI = 0",
    "TaxonomicGroup_StatusBarnacles_N - TaxonomicGroup_StatusBarnacles_NI = 0",
    "TaxonomicGroup_StatusAscidians_N - TaxonomicGroup_StatusAscidians_NI = 0",
    "TaxonomicGroup_StatusAlgae_N - TaxonomicGroup_StatusAlgae_NI = 0"
  )
))
...
#save
write.csv(con_df, file = "./output/contrasts_TaxonomicGroupStatus.csv")
```

## Adding Mod STATUS
### 
```{r, results='hide', fig.show='hide'}
# rom.Status <- rma.mv(yi, V, mods = ~ Status.AUS - 1,
#                                               random = list(~ 1|Study,
#                                                             ~ 1|Effect_ID,
#                                                             ~ 1|Species,
#                                                             ~ 1|Phylogeny),
...
# 
# 
# sum <- summary(rom.Status)
# sum_I2 <- i2_ml(rom.Status)
# sum_R2 <- r2_ml(rom.Status)
# 
# res.Status <-data.frame(effectname = attr(sum$beta, "dimnames")[[1]],
#                           beta = as.numeric(sum$beta),
#                           percentage_increase_or_decrease_on_Treatment = (exp(sum$beta)-1)*100,
#                           real.effect.value = exp(sum$beta),
#                           pval = sum$pval,
...
#                           Moderator_significance_stat = sum$QM,
#                           Moderator_significance_pval = sum$QMp,
#                           AIC = rom.Status$fit.stats$REML[3])
# 
# 
# res_Status_df <- bind_rows(res.Status)
# write.csv(res_Status_df, file = "./output/summ_Status.csv")
# 
# 
# Status_orch <-
#   orchard_plot(rom.Status, mod = "Status.AUS", xlab = "Ratio of means", group="Study_Experiment", data=meta) + theme(axis.text.y = element_text(angle = 0, hjust = 1))
# Status_orch
# 
# ggsave(
#  Status_orch,
#   file = "./output/Status_orch.tiff",
#   dpi = plot_dpi,
#   width = 10,
#   height = 8
# )
...
# 
# Status_cat <-
#   caterpillars(rom.Status, mod = "Status.AUS", xlab = "Ratio of means", group="Study_Experiment", data=meta)
# Status_cat
# ggsave(Status_cat, file = "./output/Status_cat.tiff", dpi = plot_dpi)
# 
# #test whether NI and N vary from one another
# 
# table(meta$Status.AUS)
# contrast_Status <- summary(glht(
#   rom.Status,
#   linfct = c(
#     "Status.AUSNI - Status.AUSN = 0"
#   )
# ))
# 
# contrasts_list <- mget(ls(pattern = "^contrast_+"))
...
# 
# #save
# write.csv(con_df, file = "./output/contrasts_Status.csv")

```

### test for differences in variance between N and NI
```{r}
rom.Status.het <- rma.mv(yi, V, mods = ~ Status.AUS,
                                              random = list(~ 1|Study, 
                                                            ~ 1 + Status.AUS|Effect_ID,
                                                            ~ 1|Species,
                                                            ~ 1|Phylogeny),
                                                           R = list(Phylogeny = cor),
                                                           #rho = 0,
...
# Multivariate Meta-Analysis Model (k = 1950; method: REML)
# 
#     logLik    Deviance         AIC         BIC        AICc   
# -2635.5380   5271.0760   5285.0760   5324.0979   5285.1338   
# 
# Variance Components:
# 
#             estim    sqrt  nlvls  fixed     factor    R 
# sigma^2.1  0.0972  0.3117      8     no      Study   no 
# sigma^2.2  0.0521  0.2282     60     no    Species   no 
# sigma^2.3  0.0360  0.1898     60     no  Phylogeny  yes 
# 
# outer factor: Effect_ID  (nlvls = 1950)
# inner factor: Status.AUS (nlvls = 2)
# 
#             estim    sqrt  k.lvl  fixed  level 
# tau^2.1    0.0703  0.2651    631     no      N 
# tau^2.2    0.0000  0.0004   1319     no     NI 
# 
# Test for Residual Heterogeneity:
# QE(df = 1948) = 1486.5229, p-val = 1.0000
# 
# Test of Moderators (coefficient 2):
# F(df1 = 1, df2 = 1948) = 0.0956, p-val = 0.7572
# 
# Model Results:
# 
#               estimate      se    tval    df    pval    ci.lb   ci.ub    
# intrcpt         0.1560  0.1819  0.8576  1948  0.3912  -0.2007  0.5128    
# Status.AUSNI    0.0313  0.1013  0.3091  1948  0.7572  -0.1674  0.2301    
# 
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

rom.Status.hom <- rma.mv(yi, V, mods = ~ Status.AUS,
                                              random = list(~ 1|Study, 
                                                            ~ 1|Effect_ID,
                                                            ~ 1|Species,
                                                            ~ 1|Phylogeny),
...
saveRDS(rom.Status.hom, here("data", "rom.Status.hom.RDS"))
saveRDS(rom.Status.het, here("data", "rom.Status.het.RDS"))

rom.Status.hom <- readRDS(here("data", "rom.Status.hom.RDS"))
rom.Status.het <- readRDS(here("data", "rom.Status.het.RDS"))
  
summary(rom.Status.hom)
summary(rom.Status.het)

#Estimated variances (tau values)


# het model is much better
anova(rom.Status.het, rom.Status.hom)

#         df       AIC       BIC      AICc     logLik     LRT   pval        QE 
# Full     7 5285.0760 5324.0979 5285.1338 -2635.5380                1486.5229 
# Reduced  6 5300.2813 5333.7287 5300.3246 -2644.1407 17.2053 <.0001 1486.5229 


p1 <- orchard_plot(rom.Status.hom, mod = "Status.AUS", xlab = "Ratio of means (homoscadasticiy)", group="Study_Experiment", data=meta) + 
  theme(axis.text.y = element_text(angle = 0, hjust = 1))

p2 <- orchard_plot(rom.Status.het, mod = "Status.AUS", xlab = "Ratio of means (heteroscadasticiy)", group="Study_Experiment", data=meta) + 
                     theme(axis.text.y = element_text(angle = 0, hjust = 1))

Het_hom_models_orch <- p1 + p2
Het_hom_models_orch
ggsave(
 Het_hom_models_orch,
  file = "./output/Het_hom_models_orch.tiff",
  dpi = plot_dpi,
  width = 20,
  height = 8
)
...
```{r}
rom.All <- rma.mv( yi, V, mods = ~ Status.AUS +
...
anova(rom.All, btt = 3:23)
#-> remains significant

#
summary(rom.Year)
anova(rom.All, btt = 24)
#-> remains non-significant
...
rom.All.sign <- rma.mv(yi, V, mods = ~ Feature_TaxonomicGroup,
                                              random = list(~ 1|Study,
                                                            ~ 1|Effect_ID,
                                                            ~ 1|Species,
                                                            ~ 1|Phylogeny),
...
# 
#                                                estimate      se     zval    pval    ci.lb   ci.ub      
# intrcpt                                         -0.1761  0.1789  -0.9844  0.3249  -0.5267  0.1745      
# sqrt(vi)                                         0.0348  0.2563   0.1360  0.8919  -0.4675  0.5372      
# Feature_TaxonomicGroupMovement_Anthozoans        0.0599  1.2917   0.0464  0.9630  -2.4718  2.5915      
# Feature_TaxonomicGroupMovement_Ascidians         0.9149  0.2145   4.2649  <.0001   0.4945  1.3354  *** 
# Feature_TaxonomicGroupMovement_Barnacles         0.7025  0.2040   3.4432  0.0006   0.3026  1.1024  *** 
# Feature_TaxonomicGroupMovement_Bivalves          1.5675  0.3383   4.6335  <.0001   0.9044  2.2305  *** 
# Feature_TaxonomicGroupMovement_Bryozoans         0.0121  0.1796   0.0674  0.9462  -0.3399  0.3641      
# Feature_TaxonomicGroupMovement_Polychaetes       0.0178  0.2083   0.0856  0.9318  -0.3904  0.4260      
# Feature_TaxonomicGroupMovement_Poriferans        0.1776  0.2973   0.5972  0.5504  -0.4052  0.7603      
# Feature_TaxonomicGroupOrientation_Algae         -0.4840  0.2605  -1.8580  0.0632  -0.9945  0.0266    . 
# Feature_TaxonomicGroupOrientation_Anthozoans     0.7528  0.3584   2.1001  0.0357   0.0502  1.4553    * 
# Feature_TaxonomicGroupOrientation_Ascidians      0.3799  0.2074   1.8319  0.0670  -0.0266  0.7864    . 
# Feature_TaxonomicGroupOrientation_Barnacles      0.4651  0.2033   2.2882  0.0221   0.0667  0.8635    * 
# Feature_TaxonomicGroupOrientation_Bivalves       0.4967  0.3593   1.3827  0.1668  -0.2074  1.2009      
# Feature_TaxonomicGroupOrientation_Bryozoans      0.2487  0.1806   1.3773  0.1684  -0.1052  0.6026      
# Feature_TaxonomicGroupOrientation_Polychaetes    0.5176  0.2118   2.4441  0.0145   0.1025  0.9327    * 
# Feature_TaxonomicGroupOrientation_Poriferans     0.2411  0.3275   0.7360  0.4617  -0.4009  0.8830      
# Feature_TaxonomicGroupSubstrate_Algae            0.0475  0.6682   0.0710  0.9434  -1.2622  1.3572      
# Feature_TaxonomicGroupSubstrate_Anthozoans       0.4272  0.3913   1.0918  0.2749  -0.3397  1.1941      
# Feature_TaxonomicGroupSubstrate_Ascidians        0.2535  0.2087   1.2145  0.2246  -0.1556  0.6625      
# Feature_TaxonomicGroupSubstrate_Barnacles        0.0206  0.2067   0.0996  0.9206  -0.3845  0.4257      
# Feature_TaxonomicGroupSubstrate_Bryozoans        0.2555  0.1854   1.3778  0.1683  -0.1079  0.6188      
# Feature_TaxonomicGroupSubstrate_Polychaetes      0.6152  0.2170   2.8351  0.0046   0.1899  1.0405   ** 
# 
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
